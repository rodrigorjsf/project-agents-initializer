...
Decision criteria for routing instructions to on-demand automation mechanisms.
...
For migration-candidate detection and token-impact estimation, load `automation-token-impact.md` only when Phase 1 flags an instruction block as a migration candidate.
...
| Skill (`disable-model-invocation: true`) | Zero passive | Manual — user-invoked | Heavy/rare with side effects |
| Skill (`context: fork`) | Zero parent | Isolated forked context | Context-heavy analysis |
| Hook (`command`) | Zero | Deterministic 100% | Formatting, file blocking |
| Hook (`prompt`) | Zero | LLM single-turn | Decisions needing judgment |
| Hook (`agent`) | Zero | LLM multi-turn with tools | Codebase verification |
| Hook (`http`) | Zero | Webhook delivery | Audit logging, notifications |
| Path-scoped rule (`paths:`) | Zero until match | Advisory, scoped | File-pattern conventions |
| Subagent (`skills: preload`) | Isolated | Delegated execution | Parallel/heavy analysis |
| Auto memory | First 200 lines at startup | Advisory — system-managed | Cross-session learnings |

Hook events: `PreToolUse` (can block via `hookSpecificOutput.permissionDecision`), `PostToolUse`, `Stop`, `SessionStart`, `UserPromptSubmit`, plus 17 others. Path-scoped rules accept negation: `paths: ["src/**/*.ts", "!src/**/*.test.ts"]`.
...
Skills, path-scoped rules — plugin and standalone. Hooks, subagents — plugin only (require Claude Code). Auto memory — mention only (system-managed). Check distribution type before generating suggestions; filter to supported mechanisms.
