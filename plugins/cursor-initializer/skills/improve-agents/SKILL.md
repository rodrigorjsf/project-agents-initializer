...
5. **Migrate automation candidates** — for each instruction flagged in Phase 1 as `HOOK_CANDIDATE`, `RULE_CANDIDATE`, or `SKILL_CANDIDATE`:
   ...
   - Select target mechanism: hook (`.cursor/hooks.json` — deterministic enforcement), `.cursor/rules/*.mdc` (file-pattern convention with globs), skill (domain knowledge/infrequent workflow), or subagent (isolated analysis)
   - Estimate token savings using the token impact estimation table in automation-migration-guide.md
   - This is the plugin distribution — suggest all mechanisms: hooks, `.cursor/rules/*.mdc`, skills, and subagents. Use the decision flowchart to select the best mechanism for each candidate.
...
- `.cursor/rules/` files (from automation migration): Read `${CLAUDE_SKILL_DIR}/assets/templates/cursor-rule.mdc` — use ONLY valid frontmatter: `description`, `alwaysApply`, `globs`
